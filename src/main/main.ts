...
let spotifyTokens: { accessToken: string; refreshToken: string } | null = null;
let spotifyCodeVerifier: string | null = null;
let pendingAuthData: { authCode: string; authState?: string } | null = null;
...
const createWindow = () => {
	...
	ipcMain.handle("show-context-menu", (_event, currentView: string) => {
		...
		const contextMenu = Menu.buildFromTemplate([
			...
			{
				label: "Open Spotify",
				click: () => shell.openExternal("spotify:")
			},
			{ type: "separator" },
			{
	...
	ipcMain.handle("set-spotify-tokens", (event, tokens: { accessToken: string; refreshToken: string }) => {
		spotifyTokens = tokens;
		console.log("Spotify tokens stored in main process");
		return true;
	});

	ipcMain.handle("get-spotify-tokens", () => {
		console.log("Retrieving Spotify tokens from main process:", spotifyTokens ? "available" : "none");
		return spotifyTokens;
	});

	ipcMain.handle("clear-spotify-tokens", () => {
		spotifyTokens = null;
	...
	ipcMain.handle("handle-oauth-callback", async (event, code: string, state?: string) => {
	...
	ipcMain.handle("window-drag-start", (event, { x, y }) => {
	...
	ipcMain.on("window-drag-move", (event, { mouseX, mouseY, startData, shiftKey, ctrlKey }) => {
		if (!isDragging) {
			isDragging = true;
	...
	ipcMain.on("window-drag-end", (event, { mouseX, mouseY, startData, shiftKey, ctrlKey }) => {
			...
			submenu: [
				{
					...
					click: (_menuItem, window) => {
						if (window && "webContents" in window) {
							(window as BrowserWindow).webContents.toggleDevTools();
						}
					}
				},
				{ type: "separator" },
		...
		},
		{
			...
			submenu: [
				...
				},
				{
					label: "Documentation",
					click: () => {
						shell.openExternal("https://github.com/your-repo/desktop-thing-for-spotify");
					}
				},
				{ type: "separator" },
				{
					label: "Report Issue",
					click: () => {
						shell.openExternal("https://github.com/your-repo/desktop-thing-for-spotify/issues");
					}
				}
			]
		}
